...
7.  **Display options**. En esta sección podréis configurar el branding
    ...
    a.  Los *[iconos]{.ul}* serán los que se muestren en el dispositivo
        (1). Deberéis tener un icono con los siguientes tamaños: 432px,
        324px, 216px,162px,108px.

    b.  *[Splash screen image]{.ul}*: Será la imagen que aparezca
        mientras carga la app (2).

    c.  *[Welcome screen image]{.ul}*: La imagen que se mostrará en la
        pantalla de bienvenida, para el login del usuario (3).

    d.  *[Background fill color]{.ul}*: El color de fondo en la pantalla
        de bienvenida (4).

    e.  *[Button fill color]{.ul}*: El color de fondo del botón de "Sign
        In" en la pantalla de bienvenida (5).

    f.  *[Status bar text theme]{.ul}*: El tema de la barra de estado
        superior de la app (6).

    g.  La zona *[Settings]{.ul}* (7) no se puede customizar.
